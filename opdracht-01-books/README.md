# Opdrachten Books — opdracht-01-books

**Folder:** `opdracht-01-books`

> ⚠️ Opdrachten 1.1, 1.2, 1.3, 1.4, 05, 11, 11.2, 13.1 en 13.2 maak je in **deze zelfde folder**. Geen nieuwe Vite-app aanmaken!
...
**Stap 2** – Ontvang prop `aantal` in BookCounter via `{aantal}` tussen de haakjes
...
**Stap 1** – Maak in `src` een map `Pages` met: `About.jsx`, `Contact.jsx`, `Home.jsx`, `Navigation.jsx`, `NoPage.jsx`
...
**Stap 1** – Voeg aan elk boek een `category`-property toe (Fantasy, Avontuur, Sciencefiction, Thriller, Romance)

**Stap 2** – Toon de categorie in `Book.jsx` en geef de prop door vanuit `BookList.jsx`

**Stap 3** – Maak een `categories`-array in BookList: `['Alle', 'Fantasy', 'Avontuur', 'Sciencefiction', 'Thriller', 'Romance']`
...
**Stap 4** – Voeg een `<div className="favorite-section">` toe met:
- Een button met `onClick={toggleLike}` en als inhoud: `liked ? '❤️' : '🤍'` (ternary operator)

**Stap 5** – Voeg conditionele tekst toe met `&&`:
```jsx
...
Voeg een flip card effect toe. Klik op een boek → kaart draait om en toont details op de achterkant.

**Stap 1** – Breid `data.js` uit met `description`, `year` en `pages` per boek
...
>
  <div className="book-inner">
    ...
    <div className="book-back">
      {/* achterkant: Details, jaar, pagina's, beschrijving */}
    </div>
  </div>
</article>
```

**Stap 6** – Voeg de flip-CSS toe (zie de opdracht in Canvas voor de exacte CSS)

**Stap 7** – Fix: voeg `e.stopPropagation()` toe aan het begin van `toggleLike` zodat klikken op het hartje de card niet ook laat flippen
